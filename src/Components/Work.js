...
function Work({ position, company, location, duration, logo, subjects }) {
  return (
    <article className="pt-8 border-b-2 border-dark-content pb-5 dark:border-light-content border-opacity-20 dark:border-opacity-20">
      <div className="flex justify-between items-center">
        ...
        {logo && <img src={logo} alt={`${company} logo`} className="h-18 w-20" />} 
      </div>
      ...
      {subjects && subjects.length > 0 && (
        <ul className="pt-2 pl-4 list-disc">
          {subjects.map((subject, index) => (
            <li key={index} className="text-content text-xs md:text-sm font-light">{subject}</li>
          ))}
        </ul>
      )}
    </article>
